...
interface LegalDOM {
    ...
    chkTerms?: HTMLInputElement | null;
    chkData?: HTMLInputElement | null;
    chkAI?: HTMLInputElement | null;
    btnRevoke?: HTMLElement | null;
...
const JadeCoreLegal = {
    ...
    // 2. CACHE DEL DOM (DOM SELECTORS) - Tipado explícito con la interfaz superior
    dom: {} as LegalDOM,
    ...
    initSelectors: function() {
        ...
        this.dom.form = document.getElementById('legalForm') as HTMLFormElement;
        this.dom.chkTerms = document.getElementById('chkTerms') as HTMLInputElement;
        this.dom.chkData = document.getElementById('chkData') as HTMLInputElement;
        this.dom.chkAI = document.getElementById('chkAI') as HTMLInputElement;
    ...
    registerEvents: function() {
        ...
        if (this.dom.form) {
            this.dom.form.addEventListener('submit', (e: Event) => this.handleConsentSubmit(e));
        }
    ...
    runScrollSpy: function() {
        ...
        if (currentSectionId) {
            this.dom.navLinks.forEach((link: HTMLElement) => {
    ...
    handleConsentSubmit: function(event: Event) {
        event.preventDefault();
        let isFormValid = true;

        // Asegurar que los checkboxes existan antes de leer su estado
        if (!this.dom.chkTerms || !this.dom.chkData || !this.dom.chkAI) return;
